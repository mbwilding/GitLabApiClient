...
public sealed class ProjectsClient : IProjectsClient
{
    ...
    public async Task<ImportStatus> ImportAsync(ImportProjectRequest request)
    {
        ...
        foreach (var prop in request.GetType().GetProperties())
        {
            ...
            {
                parameters.Add((prop.GetCustomAttributes(typeof(System.Text.Json.Serialization.JsonPropertyNameAttribute), false)[0] as System.Text.Json.Serialization.JsonPropertyNameAttribute).Name, prop.GetValue(request).ToString());
            }
        }
